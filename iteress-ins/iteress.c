...
void set_error(char *_program_name, error_func_t *func) {
   strcpy(program_name, _program_name);
   error_func = func;
}

void error(error_t type, char *msg, ...) {
...
void transpose_context(context_t *context) {
   ...
   for (total = i = 0; i < context->dt_count_a; i++)
      for (j = ARCHBIT; j >= 0; j--, total++) {
         ...
         for (o = 0; o < context->objects; o++)
            if (context->rows[o][i] & (BIT << j)) {
               new_context.rows[total][o / (ARCHBIT + 1)] |= (BIT << (ARCHBIT - (o % (ARCHBIT + 1))));
               new_context.supps[o]++;
            }
      }
skipout:
...
typedef void (concept_info_func_t)(data_t *intent, data_t **extent, context_t *context, concept_flags_t flags COMMA STATS_DATA_PARAM);
...
void compute_attribute_concepts(data_t *intents, context_t *context, count_t min_support COMMA STATS_DATA_PARAM) {
   ...
   for (total = j = 0; j < context->dt_count_a; j++) {
      for (i = ARCHBIT; i >= 0; i--) {
         ...
         compute_closure_plus_attr(intent, extent, extent_largest, intent_largest, context, j, BIT << i, min_support);
         INC_COUNTER(CLOSURES);
...
void init_factors(factors_t *factors, int threshold) {
...
void find_factors(context_t *context, factors_t *factors COMMA STATS_DATA_PARAM) {
   ...
   data_t *intent, *new_intent, *best_intent, *tmp_intent, *intent_parent_intent, *intent_new_parent_intent, *intent_best_parent_intent, *extent_parent_intent, *extent_new_parent_intent, *extent_best_parent_intent, *factor_intent, *intent_factor_parent_intent, *extent_factor_parent_intent;
   data_t **extent, **new_extent, **best_extent, **tmp_extent, **intent_parent_extent, **intent_new_parent_extent, **intent_best_parent_extent, **extent_parent_extent, **extent_new_parent_extent, **extent_best_parent_extent, **factor_extent, **intent_factor_parent_extent, **extent_factor_parent_extent, **factor_concepts_extent, **objects;
   ...
   *contexts = context;
   ...
   while (c >= 0) {
      ...
      while (true) {
         ...
         for (factor_concepts->iter = factor_concepts->start; factor_concepts->iter < factor_concepts->end; factor_concepts->iter = (data_t *)(factor_concepts_extent + (count_t)*factor_concepts_extent + 1)) {
               ...
               for (total = j = 0; j < context->dt_count_a; j++) {
                  for (i = ARCHBIT; i >= 0; i--) {
                     ...
                     if (!size && (c == last_c)) {
                        ...
                        compute_closure_plus_attr(new_intent, new_extent, extent, intent, contexts[c], j, BIT << i, factors->concepts.min_support);
                        INC_COUNTER(CLOSURES);
                     }
                     ...
                     if (new_size > best_size) {
                        best_size = new_size;
                        tmp_intent = new_intent;
                        new_intent = best_intent;
                        best_intent = tmp_intent;
                        tmp_extent = new_extent;
                        new_extent = best_extent;
                        best_extent = tmp_extent;
                        if (c) {
                           tmp_intent = intent_new_parent_intent;
                           intent_new_parent_intent = intent_best_parent_intent;
                           intent_best_parent_intent = tmp_intent;
                           tmp_extent = intent_new_parent_extent;
                           intent_new_parent_extent = intent_best_parent_extent;
                           intent_best_parent_extent = tmp_extent;
                           tmp_intent = extent_new_parent_intent;
                           extent_new_parent_intent = extent_best_parent_intent;
                           extent_best_parent_intent = tmp_intent;
                           tmp_extent = extent_new_parent_extent;
                           extent_new_parent_extent = extent_best_parent_extent;
                           extent_best_parent_extent = tmp_extent;
                        }
                     }
                  skip:
                     total++;
            ...
            skipout:
               if (best_size > size) {
                  ...
                  if (c) {
                     tmp_intent = intent_parent_intent;
                     intent_parent_intent = intent_best_parent_intent;
                     intent_best_parent_intent = tmp_intent;
                     tmp_extent = intent_parent_extent;
                     intent_parent_extent = intent_best_parent_extent;
                     intent_best_parent_extent = tmp_extent;
                     tmp_intent = extent_parent_intent;
                     extent_parent_intent = extent_best_parent_intent;
                     extent_best_parent_intent = tmp_intent;
                     tmp_extent = extent_parent_extent;
                     extent_parent_extent = extent_best_parent_extent;
                     extent_best_parent_extent = tmp_extent;
                  }
               } else
                  break;
            ...
            if (size > factor_size) {
               ...
               factor_extent = extent;
               extent = tmp_extent;

               if (c) {
                  tmp_intent = intent_factor_parent_intent;
                  intent_factor_parent_intent = intent_parent_intent;
                  intent_parent_intent = tmp_intent;
                  tmp_extent = intent_factor_parent_extent;
                  intent_factor_parent_extent = intent_parent_extent;
                  intent_parent_extent = tmp_extent;
                  tmp_intent = extent_factor_parent_intent;
                  extent_factor_parent_intent = extent_parent_intent;
                  extent_parent_intent = tmp_intent;
                  tmp_extent = extent_factor_parent_extent;
                  extent_factor_parent_extent = extent_parent_extent;
                  extent_parent_extent = tmp_extent;
               }
         ...
         if (c) {
            if (!new_context.table_entries)
               break;
         } else if (new_context.table_entries <= table_entries_percent)
            break;
      ...
      tmp_factor_concepts = factor_concepts;
      factor_concepts = new_factor_concepts;
      new_factor_concepts = tmp_factor_concepts;
...
void find_factors_help(int descr) {
   if (!descr)
...
#ifdef STATS
void print_stats(STATS_DATA_PARAM) {
   int i, usec_less;
...
#ifdef STATS
   fprintf(stderr, "factor %li: covered %li = %.3f%%", COUNTER(FACTORS), context->table_entries - COUNTER(UNCOVERED), 100 - (double)100 * COUNTER(UNCOVERED) / context->table_entries);
   ...
   if (argv[index][0] == '-')
      switch (argv[index][1]) {
         ...
         case 'O':
            switch (argv[index++][2]) {
               case 'a':
                  factor_output = CONCEPT_SAVE_ATTRIBS_ONLY;
                  break;
               case 'o':
                  factor_output = CONCEPT_SAVE_OBJECTS_ONLY;
                  break;
               case 'f':
                  factor_output = 0;
                  break;
         ...
         case 's':
            switch (argv[index++][2]) {
               case 'c':
                  context_sort = CONTEXT_SORT_COLS;
                  break;
               case 'r':
                  context_sort = CONTEXT_SORT_ROWS;
                  break;
               case 'b':
                  context_sort = CONTEXT_SORT_COLS | CONTEXT_SORT_ROWS;
                  break;
               default:
                  return 0;
            }
